...
against a live local instance — all 46 requests, including the full booking → payment → chat → complaint →
...
| Categories | `GET /api/categories` (public, active only), `POST/PUT /api/categories` (admin, includes `commissionPercent`), `GET /api/admin/categories` (admin, includes inactive — needed to reactivate one) |
